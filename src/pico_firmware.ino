 ...
 *  pico_firmware.ino
 *  ------------------------------------------------------------
 *  Firmware para la Pico Plus 2 (RP2350) del proyecto WRO 2026
 *  Open Challenge — Equipo NPC
 *
 *  Responsabilidades:
 ...
 *
 *  Hardware confirmado:
 *    Motor: Pololu 34:1 Metal Gearmotor 25Dx67L 12V con encoder 48 CPR
 *    Driver: Pololu VNH5019 Motor Driver Carrier
 *    Servo: Savox SC-1251MG (digital, coreless, metal gear)
 *    Ruedas: 64 mm de diámetro
 *    Batería: LiPo 3S 11.1V 3200 mAh
 *    Microcontrolador: Pimoroni Pico Plus 2 (RP2350) + PiCowbell Terminal
 *    IMU: Adafruit BNO085 (vía Qwiic / I2C)
 *
...
/* ============================================================
 *  CONSTANTES Y PARÁMETROS (los que dicen [CALIBRAR] requieren
 *  ajuste en pruebas físicas)
 * ============================================================ */
...
// Valor calibrado empíricamente en pista oficial WRO:
//   Pedida 0.5 m/s con MS_TO_PWM=200 → tiempo en 2m: 5.05s y 4.80s
//   Velocidad real promedio: 0.406 m/s
//   Ajuste: 200 × (0.5 / 0.406) = 246
constexpr float MS_TO_PWM        = 246.0;   // [CALIBRAR] refinar si pruebas lo justifican
constexpr float MAX_VELOCITY_MS  = 1.20;    // [CALIBRAR] máx velocidad permitida
...
// --- Encoder ---
// Valor calibrado empíricamente (promedio de 6 mediciones):
//   Robot empujado 1.0 m → ENC delta promedio: 2007 ticks
//   Mediciones individuales: 2000, 2006, 2003, 2000, 2024, 2012
//   Variación: 1.2% (excelente repetibilidad)
//
// Nota: el cálculo teórico (8115) no aplica porque hay una
// reducción mecánica adicional entre el motor y las ruedas
// (engranaje + diferencial trasero) que no estaba contemplada.
// El valor empírico es el correcto.
...
void setup() {
  ...
  // En Earle Philhower, podemos usar la librería Servo o PWM directo.
 ...
 *  con Earle Philhower funciona correctamente.
 * ============================================================ */
#include <Servo.h>
Servo servo;
...
/* ============================================================
 *  LOOP PRINCIPAL
 *  No usamos delay(); usamos micros() para mantener tiempos
 *  precisos. Tres tareas concurrentes:
 ...
 *  Nota: las direcciones se ajustaron empíricamente. La
 *  convención del firmware es que velocity_ms positivo
 *  signifique avance hacia adelante.
 * ============================================================ */
 ...
 *  FIN DEL ARCHIVO
 *  ------------------------------------------------------------
 *  Para validar este firmware en la Etapa 1 sin ROS 2:
 *    1. Subir al Pico desde Arduino IDE
 *    2. Abrir el monitor serial a 921600 baud
 *    3. Verás líneas POS,IMU,ENC continuamente
 *    4. Envía comandos manuales:
 *         M,0.3      → motor avanza despacio
 *         S,0.5      → servo gira a la derecha
 *         M,-0.2     → motor retrocede
 *         STOP       → detiene todo
 *    5. Verifica que ticks del ENC cambian al mover ruedas a mano
 *    6. Verifica que yaw del IMU cambia al rotar el robot a mano
 * ============================================================ */
